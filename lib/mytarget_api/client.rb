module MytargetApi
  # A class representing a connection to MyTarget. It holds the access token.
  class Client
    include Resolver
    ...
    # Is a `MytargetApi::Client` instance authorized.
    def authorized?
      !@token.nil?
    end
    ...
    def refresh_token(refresh_token = nil)
      params = {
        ...
        client_id: MytargetApi.app_id,
        client_secret: MytargetApi.app_secret,
        api_version: 2
      }
      response = API.call('oauth2/token.json', params)
      Result.process(response, :anything, nil)
    end

    # If the called method is a namespace, it creates and returns a new `MytargetApi::Namespace` instance.
    # Otherwise it creates a `MytargetApi::Method` instance and calls it passing the arguments and a block.
