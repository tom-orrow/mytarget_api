module MytargetApi
  ...
  #
  # @note `MytargetApi::Uploading` extends `MytargetApi` so these methods should be called from the latter.
  module Uploading
    # Files uploading. It uses the same faraday middleware stack as API method calls (by using `MytargetApi::API.connection`).
